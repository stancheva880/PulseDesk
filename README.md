...
attendance and fees. NestJS + Prisma on the backend, Next.js on the frontend, one npm-workspaces
monorepo.

- **Using the app** — open `docs/index.html`. Non-technical guides per role, in Bulgarian and English.
...
`NODE_ENV=production` does not stop it. The run then creates the super admin and the two languages
only, skips the demo club, its location, all three demo accounts and every row of demo domain data,
and finishes with `Demo data: skipped`.
...
| `MAIL_TRANSPORT` | in production | `console` writes reset links to stdout; `smtp` sends them. Production accepts `smtp` only. |
...
| `FRONTEND_URL` | no | Used in reset links and as the CORS fallback. |
...
The first two checks live in `configureApp()` (`backend/src/app-setup.ts`); the transport check lives
in `MailModule` (`backend/src/mail/mail.module.ts`), which defaults an unset variable to `console`.

Frontend variables (`frontend/.env.local`): `NEXT_PUBLIC_API_URL` is used;
`NEXT_PUBLIC_DEFAULT_LOCALE` appears in the compose files, `frontend/Dockerfile` and CI, but in
no `.env*.example`, and is read by nothing — `lib/i18n.ts` hardcodes `bg` and lazy-loads `en`.
...
npm run seed:data    # backend: more demo data on demand (see --help)
```
...
docs/        Client-facing HTML user guides: four role guides and a getting-started page, per language.
...
.github/     CI: typecheck, lint, migrations, both test suites, generated-contract drift check.
             `v*` tags also build and push both images to GHCR.
```
